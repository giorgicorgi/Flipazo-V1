...
| Clics registrados con IP `127.0.0.1` (23-ago-2026) | nginx en `api.flipazo.es` ponía `X-Real-IP` pero **no** `X-Forwarded-For`, y `api.py` lee `X-Forwarded-For` con `request.client.host` de reserva → la IP guardada era la del propio nginx. Solo se salvaban los clics que entran por el rewrite de Vercel (`flipazo.es/threads`), porque Vercel sí manda `X-Forwarded-For`; los que iban directos a `api.flipazo.es` perdían al visitante | `proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;` en `/etc/nginx/sites-enabled/api.flipazo.es` (backup en `/root/api.flipazo.es.bak.*`). ⚠️ **Esa config no está en git**: si se reinstala el servidor hay que volver a ponerla. `$proxy_add_x_forwarded_for` respeta la cadena, así que el `.split(",")[0]` de api.py sigue devolviendo al cliente original |
| El contador de conversión de `/mis-links` contaba bots y coincidencias (23-ago-2026) | Dos fallos que se cancelaban a medias y daban un 1% verosímil: (1) **207 de los 265 "visitantes"** eran el crawler de Meta que previsualiza el enlace al renderizar el perfil de Threads — la tabla `clicks` solo guardaba IP y fecha, sin `User-Agent`, así que no había forma de distinguirlos; (2) la conversión no tenía ventana temporal (`d.ts >= b.ts`), valía un clic en un deal semanas después — las dos "conversiones" ocurrieron a 8,5 h y 23,4 h | `analytics/bots.py` (UA + rangos de IP de Meta), columna `clicks.user_agent`, ventana de 30 min, y `/go/{slug}` solo registra los slugs de `_BIO_LINKS`. Real: 88 clics de personas y 57 visitantes, no 366 y 265. **Ojo al leer el panel**: la IP propia cuenta como visitante, y era el 31% de los clics humanos |
| `flipazo.es` → `www` responde **307 temporal** en vez de 308 permanente | Es el redirect de dominio de Vercel, que se resuelve en el **edge antes de invocar el despliegue**: la regla equivalente en `vercel.json` (con `has: host`) nunca llega a ejecutarse — comprobado el 18-ago-2026, sigue devolviendo 307 con la regla puesta. Google trata el 307 como temporal y no consolida del todo las señales en la versión www | **Solo se arregla en el panel**: Vercel → Project → Settings → Domains → `flipazo.es` → cambiar el redirect a 308 permanente. La regla de `vercel.json` se deja puesta a propósito: está inerte, pero toma el relevo si algún día se quita el redirect del panel y se asigna el apex al proyecto |
| Decathlon 7 noches sin publicar tras cambiar de feed (10→16-ago-2026) | Al pasar de id=98 a id=107 el histórico propio **empezó de cero** (`decathlon_precios` reiniciada, la vieja en `decathlon_precios_id98_backup`). La detección exige `_MIN_DIAS_DATOS = 7` días **anteriores a hoy**, así que hasta el día 17 no hubo ni una referencia: 0 ofertas no por falta de chollos sino porque eran **imposibles de calcular**. El log decía `0 deals detectados`, indistinguible de "no hay ofertas" | Aviso explícito cuando `ref` sale vacía: días acumulados y cuántos faltan (`_dias_de_historico`). **Cambiar de feed = reiniciar el reloj**: tras un cambio así, la tienda estará muda tantos días como exija su ventana, y hay que decirlo en el log o parece una avería. Al 7º día detectó 6 ofertas (−40% a −47%) |
